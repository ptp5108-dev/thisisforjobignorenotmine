...
int BLEPin =25;
int LORAPin =4;
volatile bool loraToggleState = true;
int bleLED=2;
...
void IRAM_ATTR handleButtonPressLora(){
    LORAFlag = true;
}
...
void do_send(osjob_t* j){
    ...
    if (LMIC.opmode & OP_TXRXPEND) {
      ...
      if (loraToggleState==false){
        mydata[0] = 1;
      }
      else if (loraToggleState){
        mydata[0] = 0;
      }
...
digitalWrite(bleLED, HIGH);
  ...
  pinMode(LORAPin, INPUT_PULLUP);
 BLE_setup();
 attachInterrupt(digitalPinToInterrupt(BLEPin),handleButtonPressBLE,FALLING);
attachInterrupt(digitalPinToInterrupt(LORAPin),handleButtonPressLora,FALLING);
 loraWAN_setup();
}
...
void loop() {
  if (BLEFlag){
    ...
    if (currentTime-lastInterrupt>debounceDelay){
      digitalWrite(bleLED,(!digitalRead(bleLED)));
      Serial.println("button pressed");
      if (digitalRead(bleLED)==LOW){
        pCharacteristic->setValue("OFF");
      }
      else if (digitalRead(bleLED)==HIGH){
        pCharacteristic->setValue("ON");
      }
    ...
    unsigned long currentTime = millis();
    if (currentTime-lastInterrupt>debounceDelay){
      loraToggleState= !loraToggleState;
      Serial.println("button pressed");

      if (LMIC.opmode & OP_TXRXPEND) {
        Serial.println(F("Radio busy (OP_TXRXPEND), packet queued or dropped!"));
      } else {
        // Safe to schedule a packet transmission now
        do_send(&sendjob);
      }
      lastInterrupt=currentTime;
    }
